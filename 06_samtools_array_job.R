...
q = paste0('select count(File.idSample) as files, Sample.idData, Sample.title, Sample.id as SampleID, Sample.group1 as g1 from File, Sample
           where (Sample.idData = 46 and File.idSample = Sample.id) group by File.idSample')
...
q = paste0('select File.*, Sample.idData, Sample.group1 as g1 from File, Sample 
           where (Sample.idData = 46) and (File.idSample = Sample.id) and (File.type = "fastq")')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
                   where (Sample.idData = 46 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   (File.type = "fastq")')
...
# format d-hh:mm:ss
cvRuntime = '#SBATCH --time=0-05:05:00'
