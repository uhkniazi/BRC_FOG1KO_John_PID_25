# File: 19_gseaSummaryTableFilteredMerged.R
# Auth: umar.niazi@kcl.ac.uk
# DESC: merge the gsea results for all contrasts in one table
# Date: 8/3/2021


lFiles = list.files('results/mergedDownstream/', pattern='*pathways_mSigDb_c2_*', full.names = T, ignore.case = F)
...
########
write.csv(dfMerged.c2, file='results/mergedDownstream/gsea_msigdb_c2_merged.xls')

###################### repeat for other databases
################### C3
lFiles = list.files('results/mergedDownstream/', pattern='*pathways_mSigDb_c3_*', full.names = T, ignore.case = F)
...
########
write.csv(dfMerged.c3, file='results/mergedDownstream/gsea_msigdb_c3_merged.xls')

########################
####### C5
lFiles = list.files('results/mergedDownstream/', pattern='*pathways_mSigDb_c5_*', full.names = T, ignore.case = F)
...
########
write.csv(dfMerged.c5, file='results/mergedDownstream/gsea_msigdb_c5_merged.xls')

########################################################
############ C7
lFiles = list.files('results/mergedDownstream/', pattern='*pathways_mSigDb_c7_*', full.names = T, ignore.case = F)
...
########
write.csv(dfMerged.c7, file='results/mergedDownstream/gsea_msigdb_c7_merged.xls')

#######################################################
### C8

lFiles = list.files('results/mergedDownstream/', pattern='*pathways_mSigDb_c8_*', full.names = T, ignore.case = F)
...
########
write.csv(dfMerged.c8, file='results/mergedDownstream/gsea_msigdb_c8_merged.xls')

######################################################
######### hallmark
lFiles = list.files('results/mergedDownstream/', pattern='*pathways_mSigDb_hm_*', full.names = T, ignore.case = F)
...
aheatmap(mMat, annRow = ann$DB, scale = 'none', Rowv = order(g2:g1), Colv=NA, cexRow=5, cexCol = 0.6, #labCol=c('C2vC1', 'K1vC1', 'K2vC2', 'K2vK1'), 
         col=c('white', brewer.pal(9, 'YlOrRd')))

pdf('results/mergedDownstream/gsea_msigdb_significant_merged.pdf')
aheatmap(mMat, annRow = ann$DB, scale = 'none', Rowv = order(g2:g1), Colv=NA, cexRow=5, cexCol = 0.6, #labCol=c('C2vC1', 'K1vC1', 'K2vC2', 'K2vK1'), 
         col=c('white', brewer.pal(9, 'YlOrRd')))
dev.off(dev.cur())
